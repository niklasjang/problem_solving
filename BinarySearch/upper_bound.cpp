/*
upper_bound : key값으로	정렬된 배열과 리스트가 되어있어야 한다.
key 값보다 큰 수 중 가장 작은 값을 가리키는 iterator를 반환한다.
-------------------------------------
...
key보다 큰 수 중 가장 작은 값을 가리키도록 정의했으므로,
v[mid]와 key가 같을 때 left를 mid+1로 키운다.
*/
#include <iostream>
#include <algorithm>
#include <vector>
...
int myupper_bound(int* arr, int n, int key) {
	...
	while (left < right) {
		mid = (left + right) / 2;
		if (arr[mid] <= key) { // 중간값 <=찾으려는 값 <----- lower_bound와 다른 점은 '='하나
			left = mid + 1;
		}else{ //찾으려는 값 < 중간값  // 중간값 <=찾으려는 값 <----- lower_bound와 다른 점은 '='하나
			right = mid;
		}
	}
